...
function renderModalHeader(player) {
  ...
  // record — so these two stats add nothing there. See SINGLE_GENDER above.
  const mixedWinPctStat = SINGLE_GENDER
    ? ''
    : `<div class="mh-stat"><div class="n">${formatCategoryWinPct(player.mixedWins, player.mixedLosses)}</div><div class="l">MIXED WIN%</div></div>`;
  const genderWinPctStat = SINGLE_GENDER
    ? ''
...
// Each entry is [label, duo, sign], sign being how that duo's synergy enters
// the number beside this tag (+1 added, -1 subtracted — see
// computeExpectedOutcome's margin formula). Naming each side's own synergy
// isn't enough once there are two of them: "this pair +3.1, the other pair
// +1.8" still leaves the reader to work out that those partly cancel out,
// so with more than one entry the tooltip also spells out the net effect on
// the number it's attached to.
function renderPairChemistryTag(pairs) {
  ...
  const detail = entries
    ...
    .join('; ');
  const net = Math.round(entries.reduce((total, [, duo, sign = 1]) => total + sign * duo.synergy, 0) * 10) / 10;
  const netNote = entries.length > 1 ? ` — net effect on this number: ${formatSignedValue(net, 1)} pts/game` : '';
  return ` <span class="exp-tag exp-chem" title="Adjusted for real partner chemistry — ${detail}${netNote}">chem</span>`;
}
...
function describeProjectedOutcome(expectation) {
  ...
  const chemistryTag = synergy
    ? renderPairChemistryTag([['this pair', synergy.ours, 1], ['the other pair', synergy.theirs, -1]])
    : '';
...
function renderTeamPage(team, { scroll = true } = {}) {
  ...
  const rosterColumns = [
    ...
    { key: 'diff', label: '+/–' },
    { key: 'mixedWinPct', label: 'Mixed Win%' },
    { key: 'genderWinPct', label: 'Gender Win%' },
    { key: 'gamesPlayed', label: 'GP' },
    // Mixed is always 0–0 in a single-gender division and Gender just repeats
    // the overall win%, same reasoning as the main table's COLUMNS filter.
  ].filter(({ key }) => !(SINGLE_GENDER && (key === 'mixedWinPct' || key === 'genderWinPct')));
