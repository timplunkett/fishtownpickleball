...
// 🟢 Writes the dashboard directly to cpl/index.html
const OUT = process.env.CPL_OUT || path.join(__dirname, "index.html");

// 🟢 Points one level up to the root folder to find template.html
...
// ----- main ------------------------------------------------------------------
async function main() {
  console.log("Loading local season matchups …");

  // 🟢 Points one level up to the root folder to read the files generated by fetch-data.js
  const feed = JSON.parse(fs.readFileSync(path.join(__dirname, "../_data/matchups.json"), "utf8"));
  const playerListJson = JSON.parse(fs.readFileSync(path.join(__dirname, "../_data/players.json"), "utf8"));
  const matchupDetailsJson = JSON.parse(fs.readFileSync(path.join(__dirname, "../_data/matchupDetails.json"), "utf8"));
  ...
  for (const mu of completed) {
    console.log(`Processing match wk${mu.weekNumber} ${mu.homeName} vs ${mu.awayName} … `);
  ...
  console.log("Processing division players (rankings) …");
  let rankByName = {};
  ...
  try {
    ...
    console.warn("  WARN: players calculation failed:", e.message);
  }
  ...
  if (!fs.existsSync(TEMPLATE)) {
    throw new Error(`template.html is missing from path: ${TEMPLATE}`);
  }
  ...
  // 🟢 Dynamically creates the cpl directory if running fresh on GitHub Actions
